...
import { type AdsbLol, type CTX, DbFlags } from './types'
...
import { getDb, updateDb } from './db/funcs'

const inRangePlanes = new Set<string>()

function getCategory(plane:AdsbLol['aircraft'][number]):'Military'|'Whitelisted'|null {
  const lists = getLists()

  const reg = plane.r?.toUpperCase() ?? ''
  const type = plane.t?.toUpperCase() ?? ''

  if (lists.whitelist.registration.has(reg) || lists.whitelist.type.has(type)) return 'Whitelisted'
  if (lists.blacklist.registration.has(reg) || lists.blacklist.type.has(type)) return null
  const isMilitary = ((plane.dbFlags ?? 0) & DbFlags.Military) !== 0

  return isMilitary ? 'Military' : null
}
...
async function getPlanes() {
  ...
  try {
    const res = await fetch(`https://re-api.adsb.lol/?circle=${config.coords.lat},${config.coords.lon},${config.radius}`, { signal: timeout() })
    if (!res.ok) return console.log(`Failed to fetch ${res.url}:`, res.status, res.statusText)
    const { aircraft }:AdsbLol = await res.json()
    ...
    for (const plane of aircraft) {
      ...
      const aircraft:CTX = {
        plane: {
          ...
          callsign: plane.flight?.trim() ?? apl?.flight?.trim() ?? null,
          type: db?.type ?? apl?.desc ?? `${adsbdb?.manufacturer} ${adsbdb?.type}`,
          operator: db?.operator ?? apl?.ownOp ?? adsbdb?.registered_owner,
          alt: plane.alt_baro,
...
(async () => {
  ...
  getPlanes()
  setInterval(getPlanes, config.interval * 1000)
})().catch(error => console.error(error))