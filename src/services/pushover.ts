...
export async function sendPushoverNotif({ plane, category, thumb }:PlaneContext) {
  try {
    ...
    formData.append('message', `a ${plane.desc ?? 'N/A'} at ${plane.alt_baro ?? 'N/A'}ft!`)
    formData.append('url', `https://globe.adsbexchange.com/?icao=${plane.hex}`)
    formData.append('url_title', `${plane.r} on adsbexchange`)
