import { BaseMessageOptions, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle } from 'discord.js'
import type { CTX } from '~/types'
import { config } from '~/config'
import { timeout, getAltColour, format } from '~/utils'
import { getOwm } from '.'
...
function seen(count:number, time:string):string {
  const timestamp = Math.floor(new Date(time).getTime() / 1000)
  return `${count} time${count === 1 ? '' : 's'} (<t:${timestamp}:R>)`
}

let url:string
switch (config.discord.method) {
  case 'bot':
    url = `${config.discord.bot.endpoint}/${config.discord.bot.channelId}`
    break
  case 'webhook':
    url = config.discord.webhookUrl
    break
    
  default: throw new Error('Unknown Discord method')
}
...
export async function sendToDiscord({plane, thumb, db, category}:CTX) {
  ...
  const lnkBtns = [
    ...
    },
    {
      ...
      url: plane.callsign ? `https://flightradar24.com/${plane.callsign.trim()}` : 'https://flightradar24.com',
  ...
  const payload:BaseMessageOptions = {
    ...
    embeds: [new EmbedBuilder()
      ...
      .setFields([
        ...
        { name: 'Altitude', value: format.altitude(plane) },
        { name: 'Lat Lon', value: format.coords(plane) },
        { name: 'Heading', value: format.heading(plane.heading) },
  ...
  try {
    ...
    else console.log('Sent Discord message.')
